...
class Payouts {
  ...
  async createBeneficiaryToCashhfree(body) {
    try {
      ...
      } = await this.genrateAuthorizationTokenCashfree();
      if (!token) {
  ...
  async verifyBankAccountCashfree(bankAccount, ifsc) {
    const { data: resp } = await this.genrateAuthorizationTokenCashfree();
    if (resp.subCode !== "200") {
  ...
  async createUpdateAccountDetails(id, data) {
    return partnerBankDetailsModel.findOneAndUpdate({ partnerId: id }, data, {
      new: true,
      upsert: true,
    });
  }
  ...
  async createPayoutOnDbOnline(data, categoryId) {
    ...
    if (!partnerDataRe) {
      ...
      totalDeduction += amt;
      deduction.push({
        title: "Tax collection (Without GST)",
        value: amt,
        desc: "Tax deduction",
      });
    } else {
    ...
    if (companyComissionPercentage) {
      ...
      totalDeduction += amt;
      deduction.push({
        title: "Phixmen Commision",
        value: amt,
        desc: "Phixmen Commision",
      });
    } else {
    ...
    if (codAmount !== 0) {
      deduction.push({
        title: "Partner Cash payment amount",
        value: codAmount,
        desc: "Cash payment",
      });
    }
    ...
    if (payableAmount < 0) {
      ...
      deduction.push({
        title: "This fees will be deducted in next order",
        value: totalDeduction,
        desc: "over due",
      });
      await partnerModel.findByIdAndUpdate(data.partnerId, {
        $inc: { codCollection: totalDeduction },
      });
    }
    ...
    if (leftAmount > 0) {
      ...
      if (howMuchDeduct) {
        ...
        deduction.push({
          title: "This fees is deducted againts your previous order",
          value: howMuchDeduct,
          desc: "deduction",
        });
        await partnerModel.findByIdAndUpdate(data.partnerId, {
          $inc: { codCollection: -howMuchDeduct },
        });
      }
    }
    ...
    try {
      ...
      const newwithDraw = new payoutModel({
        ...data,
        transferId,
        logs: [
          {
            status: payoutStatusTypesObject.WITHDRAW,
  ...
  async createPayoutOnDbCod(data, categoryId) {
    ...
    if (data["paymentMode"] === "cod") {
      ...
      totalDeduction += amt;
      deduction.push({
        title: "Cash payment/ self 2% Deduction",
        value: amt,
        desc: "Cash payment",
      });
    }
